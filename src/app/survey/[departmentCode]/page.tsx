...
export default function SurveyPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitted(true);
      setMessage("✅ ส่งแบบประเมินเรียบร้อยแล้ว ขอบคุณครับ");
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (err: any) {
  ...
  return (
    ...
    <div className="h-dvh overflow-y-auto overflow-x-hidden overscroll-contain relative text-slate-900">
      ...
      <header className="sticky top-0 z-40 border-b border-sky-100/80 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative h-9 w-9 sm:h-10 sm:w-10 rounded-xl overflow-hidden ring-1 ring-sky-100 bg-white">
              <Image
                src="/logos/rmu.png"
                alt="University Logo"
                fill
      ...
      >
            ...
            <div className="p-3 sm:p-4 md:p-6">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  {!userGroup && !submitted && (
                    <p className="mt-1.5 text-[11px] sm:text-[12px] text-slate-500">
        ...
        {!loading && fetchError && (
          <div className="rounded-xl border border-rose-200 bg-rose-50 text-rose-700 p-4 text-[13px] sm:text-sm">
            {fetchError}
          </div>
        )}
        ...
        {message && (
          <div
            className={`mt-6 rounded-xl border p-4 text-[13px] sm:text-sm ${
              message.startsWith("✅")
...
function QuestionCard({
  ...
  return (
    <section
      ...
      "
    >
      <p className="font-semibold text-slate-900 text-[clamp(0.98rem,1.8vw,1.06rem)] leading-6">
  ...
  return (
    <div className="space-y-2.5 sm:space-y-3 md:space-y-4">
      {[...Array(4)].map((_, i) => (
        <div
          key={i}
          className="rounded-2xl border border-sky-100 bg-white/90 p-3 sm:p-4 md:p-6 shadow-sm animate-pulse"
        >
          ...
          <div className="mt-4 grid grid-cols-5 gap-2 sm:flex sm:flex-wrap sm:gap-2.5">
            {[...Array(5)].map((__, j) => (
              <div
                key={j}
                className="h-[44px] w-full sm:w-[3.5rem] md:w-[3.75rem] lg:w-[4rem] bg-slate-100 rounded-2xl"
              />
            ))}
          </div>
        </div>
